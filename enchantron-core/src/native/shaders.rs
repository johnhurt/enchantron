...
macro_rules! shader_var {
    ...
    ($name:ident, $arg:ident : vec4_f64 = $arg_val:expr ) => {
...
macro_rules! shader_vars {
    ($name:ident, { $arg:ident : $var_type:ident $( = $var_val:expr)? }) => {
        shader_var! { $name, $arg : $var_type $( = $var_val )? }
    };
    ($name:ident, { $arg:ident : $var_type:ident $( = $var_val:expr)? $(, $more_arg:ident : $more_var_type:ident $( = $more_var_val:expr)? )+ }) => {
        shader_var! { $name, $arg : $var_type $( = $var_val )? }
...
define_shaders! {
    terrain_shader = TerrainShader {
        square_texture_height: f64,
        GREEN: vec4_f64 = u8_rgb_to_f64_rgba(&GRASS_GREEN_RGB, 255),
        BROWN: vec4_f64 = u8_rgb_to_f64_rgba(&DIRT_BROWN_RGB, 255)
    }
}
