...
impl TouchTracker {
    ...
    pub fn to_touch_event(&mut self, raw_event: RawTouchEvent) -> TouchEvent {
        match raw_event {
            RawTouchEvent {
                state: Start,
                touch,
    ...
    fn on_one_touch_start(&mut self, touch: RawTouch) -> TouchEvent {
        let touch_id = touch.touch_id;

        let touch = if self.touches[Finger1].is_none() {
            Touch::new(Finger1, touch)
        } else {
            Touch::new(Finger2, touch)
        };
    ...
    fn on_two_touch_start(
        ...
        let touch_1 = Touch::new(Finger1, touch_1);
        let touch_2 = Touch::new(Finger2, touch_2);
    ...
    fn on_one_touch_move(&mut self, raw_touch: RawTouch) -> TouchEvent {
        let finger = self.get_finger_for_id(raw_touch.touch_id);
        let touch = self.touches[finger].as_mut().unwrap().1.update(raw_touch);
    ...
    fn on_one_touch_end(&mut self, raw_touch: RawTouch) -> TouchEvent {
        let finger = self.get_finger_for_id(raw_touch.touch_id);
        let touch = self.touches[finger].as_mut().unwrap().1.update(raw_touch);
