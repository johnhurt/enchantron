    ...
    K: SlotMapKey<P>,
{
    ...
    fn write_to_existing_slot(&mut self, slot_key: &SlotMapKeyData, value: T) {
        if slot_key.chunk_index == self.current_chunk_index {
            ...
            if let Some(mut chunk) = self.storage.get_mut(slot_key.chunk_index)
            {
                if let Some(mut slot) = chunk.get_mut(slot_key.index_in_chunk) {
    ...
    fn move_current_chunk_to_storage(&mut self) {
        let mut storage_chunk = Box::new(array_macro::array![|i| {
                self.current_chunk
                    .get_mut(i)
